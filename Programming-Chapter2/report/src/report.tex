...
\subsubsection{Add an Interpolating Point}
...
You should include the header \verb|interpolation.h|. Then define an Hermite interpolation polynomial.

\begin{lstlisting}
  HermiteInterpolation poly;
\end{lstlisting}

You should give your interpolating conditions like $f^{(k)}(x)=y$ with the following code.

\begin{lstlisting}
  poly.addCondition(x, k, y);
\end{lstlisting}
...
You can get the deriviate function of $k$th order with the following code.

\begin{lstlisting}
  HermiteInterpolation dpoly = poly.diff(k);
\end{lstlisting}

\subsubsection{Output}

This package supports two output modes: Latex and Tikz.
...
The default output mode is Latex. To change it, using one of the following codes.
...
\begin{align*}
  f(x)=&75x^{1}+7.16191x^{2}-10.0953x^{3}+5.50812x^{4}-1.5383x^{5}+0.243041x^{6}\\
...
\begin{align*}
  f'(x)=&75+14.3238x^{1}-30.2859x^{2}+22.0325x^{3}-7.69148x^{4}+1.45825x^{5}\\
...
\begin{center}
  \begin{tikzpicture}[global scale = 0.8]
    ...
    \addplot+[thick,no marks]{0.1334+1.063*\x+8.241*\x*(\x-0.2)-67.3803*\x*(\x-0.2)*(\x-0.333333)+219.77*\x*(\x-0.2)*(\x-0.333333)*(\x-0.433333)-475.349*\x*(\x-0.2)*(\x-0.333333)*(\x-0.433333)*(\x-0.566667)+604.735*\x*(\x-0.2)*(\x-0.333333)*(\x-0.433333)*(\x-0.566667)*(\x-0.666667)};
